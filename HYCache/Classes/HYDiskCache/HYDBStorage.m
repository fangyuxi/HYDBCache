...
- (BOOL)removeItemWithKey:(NSString *)key
{
    ...
    if (result != SQLITE_DONE) {
        if (_logsEnabled) {
            NSLog(@"%s line:%d db delete error (%ld): %s",
                  __FUNCTION__,
                  __LINE__,
                  (long)result,
                  sqlite3_errmsg(_db));
...
- (NSArray *)removeOverdueByMaxAge
{
    ...
    do {
        ...
        if (result == SQLITE_ROW) {
            ...
            const char *value = sqlite3_column_blob(stmt, 1);
            if (!value) {
                char *cStringFileName = (char *)sqlite3_column_text(stmt, 0);
                if (cStringFileName) {
...
- (NSArray *)removeItemsByLRUWithCount:(NSUInteger)count
{
    ...
    do {
        ...
        if (result == SQLITE_ROW) {
            ...
            const char *value = sqlite3_column_blob(stmt, 2);
            if (!value) {
...
- (NSInteger)getTotalItemSize {
    ...
    if (result != SQLITE_ROW) {
        if (_logsEnabled){
            NSLog(@"%s line:%d sqlite query error (%ld): %s",
                  __FUNCTION__,
                  __LINE__,
                  (long)result,
                  sqlite3_errmsg(_db));
